...
def _delivery_event_metrics(fct_delivery_event_df: Any) -> Any:
    ...
    return fct_delivery_event_df.groupBy("p_date", "region_code", "carrier_id").agg(
        F.count(F.lit(1)).cast("long").alias("total_delivery_events"),
        F.countDistinct(F.when(F.col("event_type") == F.lit("DELIVERED"), F.col("shipment_id")))
        .cast("long")
        .alias("delivered_event_shipments"),
        F.countDistinct(
            F.when(
...
def build_kpi_delivery_daily(
    ...
    base = (
        agg_shipment_daily_df.withColumn(
            "p_date", _safe_col(agg_shipment_daily_df, "p_date", F.current_date())
        )
        .withColumn(
            "region_code",
            F.coalesce(_safe_col(agg_shipment_daily_df, "region_code", None), F.lit("UNKNOWN")),
        )
        .withColumn(
            "carrier_id",
            F.coalesce(_safe_col(agg_shipment_daily_df, "carrier_id", None), F.lit("UNKNOWN")),
        )
        .withColumn(
            "total_shipments", _safe_col(agg_shipment_daily_df, "total_shipments", 0).cast("long")
        )
        .withColumn(
            "delivered_shipments",
            _safe_col(agg_shipment_daily_df, "delivered_shipments", 0).cast("long"),
        )
        .withColumn(
            "on_time_shipments",
            _safe_col(agg_shipment_daily_df, "on_time_shipments", 0).cast("long"),
        )
        .withColumn(
            "delayed_shipments",
            _safe_col(agg_shipment_daily_df, "delayed_shipments", 0).cast("long"),
        )
        .withColumn(
            "exception_shipments",
            _safe_col(agg_shipment_daily_df, "exception_shipments", 0).cast("long"),
        )
        .withColumn(
            "avg_transit_time_hours",
            _safe_col(agg_shipment_daily_df, "avg_transit_time_hours", None).cast("double"),
        )
        .withColumn(
            "total_shipping_cost_usd",
            _safe_col(agg_shipment_daily_df, "total_shipping_cost_usd", 0.0).cast("double"),
        )
        .withColumn(
            "total_distance_miles",
            _safe_col(agg_shipment_daily_df, "total_distance_miles", 0.0).cast("double"),
        )
    )
    ...
    base = (
        base.withColumn(
            "total_delivery_events", F.coalesce(F.col("total_delivery_events"), F.lit(0))
        )
        .withColumn(
    ...
    result = (
        base.withColumn(
            "on_time_delivery_rate",
            F.when(
                delivered_denominator > F.lit(0), F.col("on_time_shipments") / delivered_denominator
            ).otherwise(F.lit(0.0)),
        )
        .withColumn(
            "avg_transit_hours",
        ...
        .withColumn(
            "late_delivery_rate",
            F.when(
                delivered_denominator > F.lit(0), F.col("delayed_shipments") / delivered_denominator
            ).otherwise(F.lit(0.0)),
        )
        .withColumn(
            "first_attempt_success_rate",
