...
def build_agg_shipment_daily(fct_shipment_df: Any) -> Any:
    ...
    agg = (
        ...
        .agg(
            ...
            F.sum(F.when(safe_delay > F.lit(0.0), F.lit(1)).otherwise(F.lit(0))).cast("long").alias("delayed_shipments"),
            F.sum(safe_exception).cast("long").alias("exception_shipments"),
            F.sum(safe_delay).cast("double").alias("total_delay_minutes"),
            F.avg(F.when(safe_delay > F.lit(0.0), safe_delay)).cast("double").alias("avg_delay_minutes"),
        ...
        .withColumn(
            ...
            F.when(F.col("delivered_shipments") > 0, F.col("on_time_shipments") / F.col("delivered_shipments")).otherwise(F.lit(0.0)),
        )
        .withColumn(
            "exception_rate",
