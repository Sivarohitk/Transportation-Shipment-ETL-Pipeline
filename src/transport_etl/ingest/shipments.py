"""Shipments ingestion with schema enforcement and row-level validation."""
...
def _require_spark() -> None:
    ...
    if F is None or T is None:
        raise ImportError("pyspark is required for shipment ingestion")


def load_shipments_schema_definition(schema_path: str | Path | None = None) -> dict[str, Any]:
    """Load the shipment schema definition JSON."""
    path = (
        Path(schema_path)
        if schema_path is not None
        else resolve_resource_path("config", "schemas", "shipments.schema.json")
    )
    if not path.exists():
        raise FileNotFoundError(f"Shipment schema file not found: {path}")

    with path.open("r", encoding="utf-8-sig") as handle:
        payload = json.load(handle)

    if not isinstance(payload, dict) or "columns" not in payload:
        raise ValueError("Shipment schema definition is invalid: missing 'columns'")
...
def build_shipments_schema(schema_def: Mapping[str, Any] | None = None) -> T.StructType:
    """Build typed Spark schema for cleaned shipment records."""
    _require_spark()
    definition = dict(schema_def or load_shipments_schema_definition())
    fields = [
...
def _cast_columns(df: DataFrame, schema_def: Mapping[str, Any]) -> DataFrame:
    ...
    for col_def in schema_def["columns"]:
        ...
        if type_name in {"double", "float"}:
            ...
            raise ValueError(f"Unsupported shipment type conversion: {type_name}")

    return df


def _validation_errors(df: DataFrame, schema_def: Mapping[str, Any]) -> list[Any]:
    """Build row-level validation expressions for shipment ingestion."""
    errors: list[Any] = [
    ...
    errors.append(
        F.when(
            ...
            & (F.col("promised_delivery_ts") < F.col("pickup_ts")),
            F.lit("invalid_time_order:promised_delivery_ts_before_pickup_ts"),
        )
    )
    errors.append(
        F.when(
            ...
            & (F.col("actual_delivery_ts") < F.col("pickup_ts")),
            F.lit("invalid_time_order:actual_delivery_ts_before_pickup_ts"),
        )
    )
...
def read_shipments_raw(
    ...
    """Read, clean, and validate shipment raw data for staging.

    Returns only valid rows, while invalid rows are optionally written to a
    quarantine path via `bad_records_path`.
    """
    _require_spark()
    schema_def = dict(schema_def or load_shipments_schema_definition())
    ...
    if bad_records_path:
        parser_path = bad_records_path.rstrip("/\\") + "/parser/shipments"
    ...
    _write_invalid_records(
        ...
        entity=str(schema_def.get("entity", "shipments")),
    ...
    LOGGER.info("Shipment ingestion completed for source_path=%s", source_path)
    return valid_df
