"""Carriers ingestion with schema enforcement and row-level validation."""
...
def _require_spark() -> None:
    ...
    if F is None or T is None:
        raise ImportError("pyspark is required for carrier ingestion")


def load_carriers_schema_definition(schema_path: str | Path | None = None) -> dict[str, Any]:
    """Load the carrier schema definition JSON."""
    path = (
        Path(schema_path)
        if schema_path is not None
        else resolve_resource_path("config", "schemas", "carriers.schema.json")
    )
    if not path.exists():
        raise FileNotFoundError(f"Carrier schema file not found: {path}")

    with path.open("r", encoding="utf-8-sig") as handle:
        payload = json.load(handle)

    if not isinstance(payload, dict) or "columns" not in payload:
        raise ValueError("Carrier schema definition is invalid: missing 'columns'")
...
def build_carriers_schema(schema_def: Mapping[str, Any] | None = None) -> T.StructType:
    """Build typed Spark schema for cleaned carrier records."""
    _require_spark()
    definition = dict(schema_def or load_carriers_schema_definition())
    fields = [
...
def _cast_columns(df: DataFrame, schema_def: Mapping[str, Any]) -> DataFrame:
    ...
    for col_def in schema_def["columns"]:
        ...
        if type_name in {"double", "float"}:
            ...
            raise ValueError(f"Unsupported carrier type conversion: {type_name}")

    return df


def _validation_errors(df: DataFrame, schema_def: Mapping[str, Any]) -> list[Any]:
    """Build row-level validation expressions for carrier ingestion."""
    errors: list[Any] = [
...
def read_carriers_raw(
    ...
    """Read, clean, and validate carrier raw data for staging.

    Returns only valid rows, while invalid rows are optionally written to a
    quarantine path via `bad_records_path`.
    """
    _require_spark()
    schema_def = dict(schema_def or load_carriers_schema_definition())
    ...
    if bad_records_path:
        parser_path = bad_records_path.rstrip("/\\") + "/parser/carriers"
    ...
    _write_invalid_records(
        ...
        entity=str(schema_def.get("entity", "carriers")),
